...
🔗 **線上 Demo**：<https://tivico.github.io/health-diet-app/>　｜　📄 **[部署說明](docs/DEPLOYMENT.md)**　｜　🗄️ **[資料庫指南](docs/DATABASE.md)**　｜　📋 **[待辦清單](docs/TODO.md)**
...
- [x] 編輯餐點（點清單項目即可修改）
- [x] 常用食物快選（最近吃過 + 台灣在地食物庫，約 50 樣）
- [x] 週 / 月統計（每日熱量長條圖、平均攝取、達標天數、體重變化）
- [x] 餐別分組（早 / 午 / 晚 / 點心）與各餐別熱量分佈
- [x] 目標體重：進度、預估達成時間、趨勢圖目標線
- [ ] 提醒推播、（之後）Supabase 同步

## 功能現況

- **今日**：熱量圓環（目標 / 已吃 / 剩餘）、三大營養素、記錄 / 編輯 / 刪除餐點（可復原、可從**最近吃過**與**台灣食物庫**快選）、依**餐別**（早 / 午 / 晚 / 點心）分組並顯示各餐小計、切換日期看歷史。
- **體重**：記錄體重 / 體脂，**可補記過去日期**，近 90 天趨勢折線圖，**目標體重**進度與預估達成時間（趨勢圖上有目標線）。
- **統計**：近 7 / 30 天每日熱量長條圖（含目標虛線）、平均攝取、有記錄天數、達標天數、體重變化、**各餐別熱量分佈**（哪一餐吃最多）。
- **衛教**：分類文章，含破除迷思與飲食障礙識別 + 台灣求助專線。
- **健康建議**：填完資料後給客製化的「怎麼吃 / 動 / 睡 / 喝水」行動建議（也可隨時再看）+ 計算公式透明呈現。
- **備份**：一鍵把所有資料匯出成文字（複製保存），貼回即可還原（跨裝置搬家、防資料遺失）；另可匯出**餐點 / 體重 CSV** 丟進 Excel、Google 試算表檢視。
- **外觀**：深色模式跟隨系統；介面為繁體中文（含日期選擇器）；自訂 App 圖示與名稱「健康飲控」。

## 安全與責任設計

- **熱量下限**：每日目標設有安全下限（女 1200 / 男 1500 大卡），不鼓勵極端節食。
- **目標體重**：低於身高對應的健康體重區間時會明確提醒並建議調整，不協助追求過低的體重。
- **預估時間誠實標示**：達成時間以線性公式估算，畫面上直接說明實際通常更慢。
...
- ✅ Git 版控；**125 項測試全過**（單元 + Widget 流程 + 真實 SQLite 整合測試）
